// Achievements for "My progress": a tiered gallery (bronze → legend) over
// MCQ practice volume, streaks, accuracy/mastery, and past-paper/Pre-Proffs
// milestones — with a "next up" highlight. Same tilt-card treatment as
// ChallengeAchievements.tsx (mouse + pen tilt with a spring, touch gets a
// tap-scale instead so it never fights page scroll); everything respects
// prefers-reduced-motion, and colors come from the app tokens.
import { useMemo, useState } from 'react';
import { AnimatePresence, motion, useMotionValue, useReducedMotion, useSpring } from 'framer-motion';
import { Lock, Sparkles, Trophy } from 'lucide-react';
import type { ProgressOverview } from '@/lib/api';
import { cn } from '@/lib/shared';
import { computeProgressAchievements, type ProgressAchievement, type ProgressCategory } from '@/lib/progressAchievements';
...
const FILTERS: Array<'All' | 'Unlocked' | 'In progress' | ProgressCategory> = ['All', 'Unlocked', 'In progress', 'Practice', 'Streaks', 'Mastery', 'Milestones'];

function Medal3D({ a, size = 44 }: { a: ProgressAchievement; size?: number }) {
...
function TiltCard({ a, isNext }: { a: ProgressAchievement; isNext: boolean }) {
...
export function ProgressAchievements({ overview }: { overview: ProgressOverview }) {
  ...
  const all = useMemo(() => computeProgressAchievements(overview), [overview]);
  ...
  return <section className="rounded-3xl border border-border bg-card p-5 sm:p-6" data-testid="section-progress-achievements">
    <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="min-w-0 flex-1">
        <h3 className="font-display text-lg leading-tight">Achievements</h3>
        <p className="text-[11px] text-muted-foreground">{earned} of {all.length} unlocked · earned by practising, keeping a streak, and improving</p>
      </div>
    ...
    <div className="mt-4 flex flex-wrap gap-1.5">
      {FILTERS.map((f) => <button key={f} type="button" onClick={() => setFilter(f)}
        ...
        data-testid={`filter-progress-achievements-${f.toLowerCase().replace(/\s+/g, '-')}`}>{f}</button>)}
    </div>
    ...
    {!shown.length && <p className="py-8 text-center text-xs text-muted-foreground">Nothing here yet — practise a bit and badges will start unlocking.</p>}
  </section>;
}
